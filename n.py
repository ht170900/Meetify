...
def insert_test_data():
    ...
    # Ensure users are updated without deleting
    cursor.execute("""
    ...
    VALUES (1, 'John Doe', 'john.doe@example.com', 'Asia/Kolkata', '2024-06-21 14:00:00', '2024-06-21 16:00:00'),
           (2, 'Jane Doe', 'jane.doe@example.com', 'Asia/Kolkata', '2024-06-21 13:00:00', '2024-06-21 14:00:00')
    ...
    # Insert meetings ensuring they are in the future
    # Insert meetings ensuring they are in the future
    cursor.execute("""
    INSERT INTO meetings (id, user_id, meeting_type, start_time, end_time, timezone, notification_interval)
    VALUES (1, 1, 'online', %s, %s, 'Asia/Kolkata', '* * * * *'),
        (2, 2, 'online', %s, %s, 'Asia/Kolkata', '* * * * *')
    ON DUPLICATE KEY UPDATE
    start_time=VALUES(start_time), end_time=VALUES(end_time), timezone=VALUES(timezone), notification_interval=VALUES(notification_interval);
    """, (future_time, future_time + timedelta(hours=1), future_time + timedelta(minutes=1), future_time + timedelta(hours=1)))
...
def send_notifications():
    ...
    for meeting in meetings:
        start_time = datetime.strptime(meeting['start_time'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=pytz.timezone(meeting['timezone']))
        if now >= start_time:
            print("Skipping past meeting or meeting that's currently in progress.")
            continue

        dnd_start = datetime.strptime(meeting['dnd_start_time'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=pytz.timezone(meeting['timezone']))
        dnd_end = datetime.strptime(meeting['dnd_end_time'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=pytz.timezone(meeting['timezone']))
        
        if dnd_start <= now <= dnd_end:
            print(f"Skipping notification for meeting ID {meeting['id']} during DND.")
            continue

        print(f"Sending notification for future meeting ID {meeting['id']}.")
        response = requests.post("https://webhook.site/5eabab59-d7c0-4029-be1d-50d7dd6c83fa", json={"message": f"Reminder: You have a meeting at {start_time}"})
